...
var notepadDefaultText =
    'Type here! Text Formatting: Bold: ctrl+b Italic: ctrl+i Underline: ctrl+u';
...
overlayBackground.addEventListener('click', function () {
    // This closes the settings page when you click off the page
...
backgroundOpacitySlider.oninput = () => {
    root.style.setProperty(
        '--background-opacity',
        backgroundOpacitySlider.value * 0.001
    );
};
...
addTabButton.addEventListener('click', function () {
    // Adds tab when you click the add tab button
    createTab();
});
...
function createTab(tabID, tabName, text, loadMode) {
    //creates a new tab
    if (!loadMode) openedNotebook.tab_counter++;
    ...
    if (text) tabWindow.textContent = text || '';
    ...
    if (!loadMode) {
        var notebook_ = notebookDatabase.find(
            (nb) => nb.id === openedNotebook.id
        );

        if (notebook_) {
            notebookDatabase
                .find((nb) => nb.id === openedNotebook.id)
...
function openTab(notepad_id, tab_id) {
    //opens a tab
    ...
    if (document.getElementById(openedTabID)) {
        ...
        document.getElementById(openedTabID).style.transform =
            'translateY(0px)';
    }
    openedTabID = tab_id;
    openedNotepad = notepad_id;

    document.getElementById(tab_id).style.filter =
        'brightness(110%) saturate(250%)';
    document.getElementById(tab_id).style.height = '90%';
    document.getElementById(tab_id).style.transform = 'translateY(-5px)';
}

function renameTab(tab_) {
    // this renames the tab
...
function saveTabs() {
    // this will save all the tabs
    //// localStorage.setItem("savedTabs", JSON.stringify(tabsData));
    // // localStorage.setItem("Tab Counter", tabCounter);
}
function saveContent(tab_id, content_) {
    // saves the specified notepad's text contents
...
function closeTab(notepad_id, tab_id) {
    ...
    if (notepadElement) notepadElement.remove();

    openedNotebook.tabs_data = openedNotebook.tabs_data.filter(
        (tab) => tab.id !== tab_id
    );
...
function openNotebook(notebook__id) {
    if (openedNotebook.id !== notebook__id) {
        ...
        openedNotebook = notebookDatabase.find(
            (notebook) => notebook.id === notebook__id
        ); //searches for the notebook we'll open in the notebook database
        ...
        if (openedNotebook.tabs_data.length === 0) {
            console.error(
                `No tabs found in ${openedNotebook.id}. Creating one for it.`
            );
            createTab();
        } else {
